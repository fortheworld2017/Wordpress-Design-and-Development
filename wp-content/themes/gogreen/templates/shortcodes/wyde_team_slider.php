    ...
    <div <?php echo gogreen_get_attributes( $layout_attrs );?>>
        ...
        <div class="team-member<?php echo esc_attr($col_name);?>" >
            ...
            if( $image ){                
                ...
                echo sprintf('<div id="%s-%s">%s</div>', $popup_id, get_the_ID(), $image);
            }
            ...
            <div class="member-name"<?php echo !empty($color)? ' style="color:'.$color.';"':''; ?>>
                <h1 style="font-size: 28px; font-family: 'Titillium Web', sans-serif; color: #36c5f0;">          
                    <?php echo esc_html( $name ); ?>
                </h1>
                <h6 style="font-size: 18px; font-family: 'Titillium Web', sans-serif;"><?php echo get_post_meta( get_the_ID(), '_w_member_detail', true ); ?></h6>
                <?php/*<h6 style="font-size: 18px; font-family: 'Titillium Web', sans-serif;"><?php echo esc_html( $position ); ?></h6>*/?>
            </div>  
            <?php echo sprintf('<a class="w-mybutton cover-image1 large " style="border-color:#ffffff;background-color:#ffffff;"  myid="%s-%s" href="#team-member-%s-%s"><span>%s</span></a>', $popup_id, get_the_ID(),$popup_id, get_the_ID(), $more );?>
            <?php endif; ?>
            <div id="team-member-<?php echo esc_attr($popup_id);?>-<?php echo esc_attr( get_the_ID() );?>" class="popup-content" >
                <div class="member-content clear" style="background-color:#36c5f0;">
                    <a href="#" class="w-close-button"></a>                    
                    <div class="member-detail">
                        <h1 style="font-size: 28px; font-family: 'Titillium Web', sans-serif; color: #ffffff;"><?php echo esc_html( $name ); ?></h1>
                       <?php /* <h4 class="member-meta"><?php echo esc_html( $position ); ?></h4> */?>
                        <?php echo '<p class="social-link">'. $social_content .'</p>'; ?>
                        <div class="member-desc" style="color:white;"><?php echo get_post_meta( get_the_ID(), '_w_member_detail', true ); ?></div>
                    </div>               
                </div>
            </div>
        </div>
