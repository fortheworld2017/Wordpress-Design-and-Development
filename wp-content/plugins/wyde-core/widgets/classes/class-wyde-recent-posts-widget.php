...
class Wyde_Widget_Recent_Posts extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
            'w-recent-posts', 
            esc_html__('Wyde Recent Posts', 'wyde-core'), 
            array(
                'classname' => 'wyde_widget_recent_posts', 
                'description' => esc_html__('Most recent posts with thumbnails.', 'wyde-core')
            )
        );
	...
	function widget($args, $instance) {
		...
		echo do_shortcode(sprintf('[wyde_recent_posts count="%s" show_date="%s"]', esc_attr( $count ), esc_attr( $show_date ) ) );
	...
	function form( $instance ) {
        ...
        $defaults = array(
            'title' => esc_html__('Recent Posts', 'wyde-core'), 
            'count' => 5, 
            'show_date' => true
        );
